...
class LibMemory {
  ...
  public static function ram() {
		...
		$result['percentage_buffers'] = round($buffers / $total * 100);
		
		if ($result['percentage_buffers'] >= '80')
			$result['alert_buffers'] = 'danger';
		else if ($result['percentage_buffers'] >= '70')
			$result['alert_buffers'] = 'warning';
		else
			$result['alert_buffers'] = 'success';
		
		$result['cached'] = $cached;
		$result['percentage_cached'] = round($cached / $total * 100);

		if ($result['percentage_cached'] >= '80')
			$result['alert_cached'] = 'danger';
		else if ($result['percentage_cached'] >= '70')
			$result['alert_cached'] = 'warning';
		else
			$result['alert_cached'] = 'success';
